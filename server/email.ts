...
import { generateV2ProfilePdf, type V2PdfInput } from './pdfGeneratorV2.js';
...
// ---------------------------------------------------------------------------
// GAP-267 Phase 2 (Part 5) -- v2 welcome email with the deterministic readout PDF
// ---------------------------------------------------------------------------

export async function sendV2WelcomeEmail(data: {
  email: string;
  readout: V2PdfInput['readout'];
  exploreThemes: V2PdfInput['exploreThemes'];
}): Promise<boolean> {
  const client = getResend();
  if (!client) {
    console.warn('[email] Resend not configured -- skipping v2 welcome email');
    return false;
  }
  ...
  let pdfBase64: string | null = null;
  try {
    pdfBase64 = await generateV2ProfilePdf({ readout: data.readout, exploreThemes: data.exploreThemes });
  } catch (pdfErr) {
    console.error(`[email] v2 PDF generation failed for ${data.email}:`, pdfErr);
  }
  ...
  try {
    await client.emails.send({
      ...
      to: [data.email],
      bcc: ['andy@sovereignassets.org'],
      subject: 'Your investor-approach profile',
      ...(pdfBase64
        ? { attachments: [{ filename: 'Your-Vector-Investor-Profile.pdf', content: pdfBase64 }] }
        : {}),
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 24px; color: #1a1a2e;">
          <h1 style="font-size: 24px; font-weight: 700; margin-bottom: 8px;">Your investor-approach profile</h1>
          <p style="font-size: 14px; color: #666; margin-bottom: 32px;">Vector by Sovereign Signal</p>
          <p style="font-size: 15px; line-height: 1.7; color: #333; margin-bottom: 16px;">
            Thank you for completing Vector. Your profile report is attached -- it describes how you approach investing, drawn only from the answers you gave. It is an orientation, not financial advice.
          </p>
          <div style="text-align: center; margin: 32px 0;">
            <a href="https://www.stackmotiveapp.com" style="display: inline-block; padding: 14px 32px; font-size: 15px; font-weight: 600; color: #fff; background-color: #6366f1; border-radius: 8px; text-decoration: none;">
    ...
    console.log(`[email] v2 welcome email sent to ${data.email}${pdfBase64 ? ' (with PDF)' : ' (without PDF)'}`);
    return true;
  } catch (err) {
    console.error('[email] Failed to send v2 welcome email:', err);
    return false;
  }
}
